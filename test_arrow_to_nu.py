...
mod tests_arrow_to_nu {
    ...
    fn test_arrow_batch_to_nu_values_primitives() {
        ...
        let float_arr = Float64Array::from(vec![Some(3.14), Some(-2.5), None]);
        ...
        assert_eq!(row0.get("float_col").unwrap(), &Value::float(3.14, span));
    ...
    fn test_arrow_batch_to_nu_values_datetime() {
        let schema = Arc::new(Schema::new(vec![
            ...
            Field::new("ts_col", DataType::Timestamp(TimeUnit::Microsecond, None), true),
        ]));
        ...
        match row1.get("ts_col").unwrap() {
            Value::Date { val, .. } => {
                assert_eq!(val.format("%Y-%m-%d %H:%M:%S").to_string(), "1970-01-01 00:00:01");
            }
            _ => panic!("Expected Date value"),
        }
    }
    ...
    fn test_arrow_batch_to_nu_values_nested() {
        ...
        let schema = Arc::new(Schema::new(vec![
            Field::new("list_col", DataType::List(Arc::new(Field::new("item", DataType::Int64, true))), true),
            Field::new("struct_col", DataType::Struct(Fields::from(inner_fields.clone())), true),
        ]));
